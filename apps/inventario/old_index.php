...
if (isset($_GET['delete_obj'])) {
    $id_borrar = intval($_GET['delete_obj']);

    $stmt = $conn->prepare("DELETE FROM inv_objetos WHERE id = ?");
    if (!$stmt) {
        die("❌ Error preparando el borrado: " . $conn->error);
    }

    $stmt->bind_param("i", $id_borrar);
    if (!$stmt->execute()) {
        die("❌ Error al intentar borrar en MySQL: " . $stmt->error);
    }

    $stmt->close();
    header("Location: index.php?tab=objetos");
    exit();
}
// if (isset($_GET['delete_obj'])) {
//     $stmt = $conn->prepare("DELETE FROM inv_objetos WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['save_obj'])) {
        $id = $_POST['id'] ?? null;
        // Fusionamos los campos de nombre para simplificar la creación manual
        $titulo = $_POST['titulo'];
        $juego = ($_POST['tipo'] == 'Juego') ? $titulo : '';
        $peli = ($_POST['tipo'] == 'Peli') ? $titulo : '';
        $objeto = ($_POST['tipo'] == 'Objeto' || empty($_POST['tipo'])) ? $titulo : '';

        $stmt = $id
            ? $conn->prepare("UPDATE inv_objetos SET objeto=?, juego=?, peli=?, localizacion=?, descripcion=?, tipo=?, plataformas=?, portada_http=? WHERE id=?")
            : $conn->prepare("INSERT INTO inv_objetos (objeto, juego, peli, localizacion, descripcion, tipo, plataformas, portada_http) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");

        if ($id) $stmt->bind_param("ssssssssi", $objeto, $juego, $peli, $_POST['localizacion'], $_POST['descripcion'], $_POST['tipo'], $_POST['plataformas'], $_POST['portada_http'], $id);
        else $stmt->bind_param("ssssssss", $objeto, $juego, $peli, $_POST['localizacion'], $_POST['descripcion'], $_POST['tipo'], $_POST['plataformas'], $_POST['portada_http']);
...
// Guardamos las locs en un array para el formulario desplegable
$loc_options = [];
while($row = $localizaciones->fetch_assoc()) {
    $loc_options[] = $row['nombre'];
}
$localizaciones->data_seek(0); // Resetear puntero
?>
<!DOCTYPE html>
...
<head>
    <style>
        ...
        .memento-img { height: 250px; object-fit: cover; background: #222; }
...
<body>
  ...
  <main class="containerB">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <a href="csv.php"><button class="btn btn-outline-info">Impotar</button></a>
    </div>
    ...
    <?php if ($tab == 'objetos'): ?>
    <div class="row row-cols-1 row-cols-md-3 row-cols-lg-5 g-4 col-md-12">
        <?php while($obj = $objetos->fetch_assoc()):
            $titulo = array_filter([$obj['juego'], $obj['peli'], $obj['objeto']])[0] ?? 'Sin Título';
                ...
                <form class="modal-content" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-6 mb-2">
                                <label>Localización (Vinculada)</label>
                    ...
                    <div class="modal-footer justify-content-between">
                        <a href="?delete_obj=<?php echo $obj['id']; ?>" class="btn btn-outline-danger" onclick="return confirm('¿Seguro que quieres borrar este objeto?')">Borrar Objeto</a>
                        <!-- <a href="?action=delete_obj&id=<?php echo $obj['id']; ?>" class="btn btn-outline-danger" onclick="return confirm('¿Borrar?')">Borrar</a> -->
    ...
    <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4 col-md-12">
        ...
        <div class="modal fade" id="editLoc<?php echo $loc['id']; ?>" tabindex="-1">
            <div class="modal-dialog">
                <form class="modal-content" method="POST">
                    ...
                    <div class="modal-footer justify-content-between">
                        <a href="?delete_loc=<?php echo $loc['id']; ?>" class="btn btn-outline-danger" onclick="return confirm('¿Borrar localización?')">Borrar localización</a>
